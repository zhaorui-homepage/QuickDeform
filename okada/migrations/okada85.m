...
%        [uE,uN,uZ] = okada85(E,N,2,30,-60,5,2,0,0,1,'plot'); 
...
% 
%       row1=603;col1=676;
%       xx=linspace(-10,10,row1);yy=linspace(-10,10,col1);
%       [E,N] = meshgrid(xx,yy);
% 	    [uE,uN,uZ] = okada85(E,N,2,30,-60,5,2,0,0,1,'plot');      
%       the=38.7533;arfa=360-10.957148;
%       B=[sind(the)*sind(arfa) -sind(the)*cosd(arfa) cosd(the)];Ifm_L=B*[uN(:)'; uE(:)'; uZ(:)'];
%       Ifm_L=reshape(Ifm_L,col1,row1);
%       figure;surf(E,N,Ifm_L); colorbar;title('Synthetic ALOS image');shading interp
%       figure;imagesc(fliplr(Ifm_L'));axis image;colorbar;title('Synthetic ALOS image');
% the1=23;arfa1=360-168.1110504;
% A=[sind(the1)*sind(arfa1) -sind(the1)*cosd(arfa1) cosd(the1)];Ifm_C=A*[uN(:)'; uE(:)'; uZ(:)'];
% Ifm_C=reshape(Ifm_C,col1,row1);
% figure;imagesc(fliplr(Ifm_C'));axis image;colorbar;title('Synthetic Envisat image');
...
%
%	Copyright (c) 1997-2010, Fran�ois Beauducel, covered by BSD License.
%	All rights reserved.
%
